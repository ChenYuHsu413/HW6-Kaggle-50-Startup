...
VISUALIZATION_DIR = os.path.join("plots", "crisp_dm_v2")
...
def save_feature_selection_performance(results_df, output_dir):
    ...
    csv_path = os.path.join(output_dir, "feature_selection_performance.csv")
...
def save_sequential_feature_selection_outputs(selection_df, output_dir):
    ...
    csv_path = os.path.join(output_dir, "sequential_feature_selection_list.csv")
...
def save_marketing_vs_administration_outputs(comparison_df, output_dir):
    ...
    csv_path = os.path.join(output_dir, "marketing_vs_administration_comparison.csv")
    table_df.to_csv(csv_path, index=False)
...
def save_business_guided_feature_selection_outputs(selection_df, output_dir):
    ...
    csv_path = os.path.join(
        output_dir, "business_guided_feature_selection_summary.csv"
    )
    table_df.to_csv(csv_path, index=False)
...
def save_all_feature_selection_algorithms_outputs(results_df, output_dir):
    ...
    csv_path = os.path.join(
        output_dir, "feature_selection_performance_allinone.csv"
    )
    table_df.to_csv(csv_path, index=False)
    ...
    best_csv_path = os.path.join(
        output_dir, "feature_selection_performance_allinone_best.csv"
    )
...
save_data_understanding_plots(df, VISUALIZATION_DIR)
print(f"\nSaved data understanding visualizations to: {VISUALIZATION_DIR}")
...
(
    ...
    sequential_selection_df, VISUALIZATION_DIR
)
...
) = save_marketing_vs_administration_outputs(marketing_admin_df, VISUALIZATION_DIR)
...
(
    ...
    business_guided_df, VISUALIZATION_DIR
)
...
(
    ...
    all_selection_results, VISUALIZATION_DIR
)
...
print(f"\nSaved model evaluation visualizations to: {VISUALIZATION_DIR}")
